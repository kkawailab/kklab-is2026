...
- `attendance01.html` — student-facing quiz. On an all-correct submission it fetches server time (never the device clock), computes a confirmation code, and shows a result screen the student screenshots and submits via Teams.
...
- The `pageId` passed to `computeCode` must match between an attendance page (`PAGE_ID` constant, e.g. `"attendance01"`) and the hardcoded value in `verify.html`. A new attendance page (`attendance02.html`, …) needs its own `PAGE_ID` and corresponding verify support, plus a link from `index.html`.
- New attendance pages should keep the HTTPS-redirect snippet in `<head>` and load `code.js`.
